    ...
    def total(self):
        ...
        for card in self.cards:
            if card.value == BJ_Card.ACE_VALUE:
                constain_ace = True
        # если на руках есть туз и сумма очков не превышает 11, будет считать туз за 11 очков
        if constain_ace and t <= 11:
...
class BJ_Player(BJ_Hand):
    ...
    def retires(self):
        print(self.name, "выбывает, так как имеет бюджет меньше 100$.")
    # ставка игрока
    def rates(self):
        self.rate = 0
        print("Принимается ставка игрока:", self.name)
        while True:
            print("У вас", self.ratess, "$")
            self.rate = int(input("Ваша ставка: "))
            if self.rate > self.ratess:
                print("Ставка превышает Ваш бюджет.")
            else:
                break
        self.ratess -= self.rate
        return self.rate
...
class BJ_Game(object):
    ...
    def play(self):
        ...
        all_money = 0
        for player in self.players:
            player_money = player.rates()
            all_money += player_money
        print("Текущий бюджет игры:", all_money)
            ...
            # выигрывают все, кто еще остался в игре
                ...
                for player in self.still_playing:
                    ...
                    if player.total < self.dealer.total:
                        ...
                        player.push()
            # ставка победителя
            for player_winner in players_winner:
                player_winner.ratess += player_winner.rate * 2
            # выбывают игроки с маленьким бюджетом
        for player in self.players:
            if player.ratess < 100:
                player.retires()
    ...
    while again != "n":
        ...
        if again == "y":
            if len(game.deck.cards) < 30:
                print("Количество карт в колоде:", len(game.deck.cards), "меньше 30.")
                print("Я обновлю колоду и перемешаю.")
                game.deck.populate()
                game.deck.shuffle()
main()
